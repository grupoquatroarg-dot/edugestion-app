...
router.put('/payment-methods/:id', requirePermission('settings', 'edit'), validate(nameSchema), async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      ...
      [name, tipo || 'Efectivo', activo !== undefined ? activo : 1, Number(req.params.id)]
    );
    return sendSuccess(res, null, 'Método de pago actualizado');
  } catch (error: any) {
...
router.delete('/payment-methods/:id', requirePermission('settings', 'delete'), async (req, res) => {
  if (!isPostgresConfigured()) {
  ...
  try {
    const pool = getPostgresPool();
    await pool.query('DELETE FROM payment_methods WHERE id = $1', [Number(req.params.id)]);
    return sendSuccess(res, null, 'Método de pago eliminado');
  } catch (error: any) {
    return sendError(res, error.message || 'Error al eliminar método de pago', 400);
  }
});
